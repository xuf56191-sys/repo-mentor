...
## 2026-08-29：目标限定文档加载与切分

### 今天的目标

学习 Document 元数据、源文件路径、起止行号和代码切分边界，
实现一个不绕过安全读取器、只处理当前学习模块候选文件的 Loader。

### 今天完成

* 新增 `retrieval_models.py`，定义 `ScopedDocument`、`DocumentChunk`、
  `DocumentLoadResult`、`RetrievalHit` 和 `RetrievalResult`；
* 新增 `document_loader.py`；
* `load_documents()` 合并 `LearningTask.evidence_sources` 和
  `rank_target_files(TargetTask)` 候选；
* 只准入 README、docs、Python、相关测试和配置文件；
* 复用现有仓库路径校验、忽略规则、符号链接、二进制判断和文件大小上限；
* 当前模块默认最多加载 20 个文件、200,000 字符；
* 使用规范仓库路径稳定生成 `repository_scope_id`；
* 使用目标、学习任务和已批准证据路径生成 `module_scope_id`；
* Markdown 按标题、Python 按顶层类/函数、配置按 section 切分；
* 过长语义片段再按连续行窗口分割；
* 修复 Pydantic 自动清理开头空行导致行号偏移的问题；
* 6 个 Loader 测试全部通过。
...
> 哪些文件在当前仓库和学习模块中有资格被检索？

它不接收一个自由问题后临时绕过范围去读取新文件。
安全和学习相关性在进入 Retriever 之前已经确定。

### 2. 路线证据是最高优先级种子

`LearningTask.evidence_sources` 来自已经生成并批准的学习路线，
因此优先级高于普通路径关键词匹配。
其他文件由现有 `rank_target_files()` 提供候选，
但仍必须通过文件类型、安全和预算过滤。

### 3. 为什么切块必须保留行号

如果片段只有内容没有路径和行号，
用户无法在仓库中复核，测验和任务也无法证明来源真实。

测试使用 `line_start` 和 `line_end` 重新从文件切片，
并断言还原内容与 `DocumentChunk.content` 一致。
这比只断言“行号字段存在”更有意义。

### 4. Pydantic 的字符串清理也可能破坏证据

RepoMentor 的 `StrictModel` 默认会清理字符串首尾空白。
这对标题和路径有益，但对需要保真的源文本会删除开头空行，
导致 AST 行号与原文不一致。

`ScopedDocument.content` 因此局部设置 `strip_whitespace=False`，
而不是关闭所有模型的字符串清理。

### 5. 范围 ID 为什么要稳定又敏感于变更

同一仓库路径应始终得到同一 `repository_scope_id`，
便于复用和隔离索引。
但学习任务或已批准证据路径变化时，
`module_scope_id` 必须变化，否则旧模块片段可能进入新任务。

### 当日验收

* 二进制、忽略目录、敏感文件、图片和过大文件被排除；
* 文件数预算能截断加载；
* 所有片段都有路径和有效行号；
* Python 函数片段可以按行号还原；
* 相同输入产生稳定范围 ID，已批准证据变化会产生新模块 ID。

8 月 29 日验收标准已满足。

### 面试核心回答

> 我的 Loader 不是遍历全仓并一股脑切块。
> 它以当前 LearningTask 的证据路径为强种子，
> 再合并 TargetTask 相关文件排序，通过路径、类型、大小和预算过滤后才读取。
> 切块优先保留 Markdown 标题和 Python 类/函数边界，
> 每块保存仓库、学习模块、源路径和真实行号。
...
理解向量检索对自然语言改写的作用，
以及关键词检索对函数名、类名、路径和精确术语的作用。
将两者组合成只能在当前仓库和学习模块中工作的 Retriever。

### 今天完成

* 新增 `hybrid_retriever.py`；
* 实现中英文分词、snake_case/CamelCase 拆分和中文 2/3-gram；
* 实现受控的 Agent 领域概念扩展；
* 使用稳定特征哈希生成离线向量并计算余弦分数；
* 实现内容、路径、标题/符号和精确 identifier 关键词得分；
* 自然语言查询提高向量权重，精确代码标识符查询提高关键词权重；
* 返回组合分、向量分、关键词分、真实源路径和行号；
* 强制使用 `repository_scope_id + module_scope_id` 过滤片段；
* 对纯向量命中使用更高充分性阈值，避免哈希碰撞误报；
* 新增 `retrieval_evaluation.py` 真实来源评测；
* 10 个学习问题的真实预期来源全部进入 Top-3；
* 巧克力蛋糕问题没有仓库证据，正确返回证据不足；
* 检索相关 13 个测试、全项目 184 个测试全部通过。
...
仓库中可能使用 `checkpoint`、`interrupt`、`resume` 和 `thread_id`。
自然语言和代码术语不完全一致，向量或概念扩展可以帮助连接这些表达。
...
函数名是精确标识符，不应被语义模糊匹配稀释。
关键词和 symbol 精确命中应获得更高权重。

### 2. 当前的向量能力是什么

当前实现是一个离线可重复基线：

1. 拆分中英文、代码标识符和中文 n-gram；
2. 根据受控概念组扩展 checkpoint、证据、检索、掌握度等领域词；
3. 将特征稳定哈希到固定维度向量；
4. 归一化后计算余弦相似度。

它不是预训练神经 Embedding，不应宣称具有通用语义理解能力。
选择这个基线的原因是离线、不上传私有仓库、不产生 API 成本、
测试结果可完全复现。

### 3. 证据充分性不能等于“总能返回 Top-K”

任何相似度系统都可以从候选中排出一个第一名，
但第一名不一定真的相关。

初次评测中，蛋糕问题因为特征哈希碰撞获得约 0.16 向量分，
如果仅按普通最低阈值就会误判为有证据。
最终规则是：有关键词证据时使用普通阈值；
没有任何关键词重合时，纯向量分必须达到更高门槛。

### 4. 为什么要保留分项得分

如果只返回 `score=0.63`，很难解释排名原因。
`RetrievalHit` 同时返回 `vector_score`、`keyword_score` 和组合分，
可以看出某个结果是因为语义概念相近，还是函数名精确匹配。

这也便于评测时调整通用权重，
而不是看到某道题失败就硬编码预期文件路径。

### 5. 10 问题评测比单元测试多验证了什么

单元测试能证明某个分数函数按预期运行，
但不能证明真实仓库中的排名质量。

`retrieval_evaluation.py` 使用 RepoMentor 自身的真实文件，
要求 10 个学习问题的预期真实来源进入 Top-3，
同时要求无关问题被拒绝。

第一次评测是 9/10 并且无关问题误报，
证明“基础测试通过”不等于“检索质量达标”。
通过增加通用精确 symbol 加分和纯向量充分性门槛，
最终达到 10/10 并正确拒绝无关问题。

### 当日验收

* 语义改写可以找到 checkpoint 恢复证据；
* `build_adaptive_graph`、`EvidenceBudget`、`_segment_starts` 等精确符号优先命中定义；
* 不同仓库或学习模块的片段被过滤；
* 返回 Top-K 片段、源路径、行号和分项得分；
* 10 个学习问题的真实来源全部进入 Top-3；
* 无关问题返回证据不足；
* 全项目 184 个测试通过。
...
> 我没有只用向量相似度，因为代码仓库中函数名、类名和路径是强精确信号。
> 我将本地向量分数和关键词/符号分数组合，并保留两个分项分数便于解释和调参。
> Retriever 只接收 Loader 已准入的片段，并强制过滤 repository_scope_id 和 module_scope_id。
> 我还设置了证据充分阈值，因为排名第一不代表真的有证据。
> 最后用 10 个学习问题做真实来源 Top-3 评测，并用一个无关问题测试拒绝能力。
...
* 定义 `SourceCitation` 和 `GroundedLearningAnswer`；
* 实现 `answer_learning_question()`；
* 命中时返回文件、起止行和原文摘录；
* 无可靠证据时返回固定拒答，不用常识猜仓库事实；
* 测试真实引用、无关问题和空问题。

### 核心理解

Retriever 负责寻找候选证据，Answerer 负责在证据边界内组织回答。
有 Top-1 不等于有答案，因此 `evidence_sufficient` 是生成之前的门禁。
引用不是装饰字段：测试会验证 excerpt 确实来自命中片段。

### 面试回答

> 我通过“先判定证据充分性、再生成、最后携带可定位引用”降低幻觉。
> 没有证据时宁可拒答，也不让模型用通用知识冒充仓库事实。

## 2026-09-01：检索接入路线与测验

### 完成内容

* 增加 `LearningEvidenceLayer`，保存一次加载和切分结果；
* 路线、测验和学习问答复用同一组 `DocumentChunk`；
* `AgentState` 增加独立 `retrieval_chunks` 字段；
* 路线生成可接收检索上下文，测验生成合并同源证据；
* 证据按路径和片段去重，不覆盖已有 State 更新。

### 核心理解

“复用同一证据层”不是三个函数各自调用一次 Loader，而是先建立范围，
之后只在已有 chunks 上建立轻量 Retriever 视图。State 节点返回局部更新，
因此新增证据不能顺手重建整份 State。

### 面试回答

> 我把加载/切分与检索查询分开缓存。三个消费者共享范围 ID 和 chunks，
> 既避免重复 IO，也防止路线、测验和问答使用不同证据口径。

## 2026-09-02：V0.8 评测与边界

### 完成内容

* 建立公开的 `evaluation/retrieval_cases.json`；
* 20 个案例包含 16 个来源问题和 4 个越界问题；
* 统计 Top-3 命中率、拒答率和越界错误回答率；
* 当前结果为 Top-3 100%、拒答 100%；
* README 公开本地特征哈希不等于神经 Embedding。

### 核心理解

检索评测必须同时测“该回答时能找到”和“不该回答时能拒绝”。
只测命中率会鼓励系统对所有问题都返回内容；只测拒答则可能得到一个什么都不答的系统。

### 面试回答

> 我的评测分为正例来源命中和负例拒答，两类都有明确分母。
> 评测文件公开，结果可离线复现，不用主观查看几个聊天示例。

## 2026-09-03：目标 Issue 数据模型

### 完成内容

* 定义 `TargetIssue`：标题、描述、labels、预期结果、deadline 和 reference；
* 增加宽松表单检查 `review_target_issue_input()`；
* 关键描述不足时返回字段级问题；
* 只接收手动资料，不自动抓取或修复 Issue。

### 核心理解

外部输入不应直接进入 Agent 业务模型。先用宽松 dict 收集，再检查缺失，
最后用 Pydantic 建立严格对象，才能让后续节点依赖稳定字段。

### 面试回答

> 我把“用户还没填完整”和“领域对象非法”分成两层，前者产生澄清问题，
> 后者由严格模型拒绝，避免把半成品 Issue 传入分析链路。

## 2026-09-04：贡献差距与准备度

### 完成内容

* 实现 `analyze_contribution_gap()`；
* 输出目标能力、已掌握、待补知识、文件和实践；
* 固定五项准备度公式：20 + 35 + 25 + 10 + 10；
* Pydantic 校验总分等于分项和且满分为 100；
* 测试更多已验证技能会确定性提高分数。

### 核心理解

准备度不是“让 LLM 看完后报 78%”。可解释评分应明确输入证据、权重、
分项和阈值，并承认它衡量的是能力/证据覆盖，不是 PR 合入概率。

### 面试回答

> 我的准备度是确定性评分卡。LLM 可以帮助提取候选要求，
> 但最终分数必须由公开分项复算，并展示每一项为什么得分或失分。

## 2026-09-05：openEuler 本地贡献模式

### 完成内容

* 实现 `build_openeuler_contribution_plan()`；
* 受控读取 README、CONTRIBUTING 和项目元数据；
* 输出环境、规范和前置知识三类清单；
* 未确认项明确标记 `needs_confirmation`；
* 目录超过预算时要求缩小到目标小模块。

### 核心理解

领域模式不是换一个 Prompt 名称，而是增加领域输入、资料优先级、
验收清单和安全边界。社区 Issue、SIG 和 CI 状态没有进入本地证据时不能被假设。

### 面试回答

> openEuler 模式是本地、手动资料驱动的贡献准备模式，
> 不直接从内核全仓开始，也不自动访问社区或修改代码。

## 2026-09-06：真实案例验证

### 完成内容

* 使用本地 openGauss Plugin #1626 实际资料形成案例；
* 对照人工复现/定位路线与 RepoMentor 准备路线；
* 记录 4 个有效点和 4 个限制；
* 诚实说明历史人工过程未统一计时，不能宣称节省小时数；
* 暴露 C++、SQL、Java 尚未进入 V0.8 索引的问题。

### 核心理解

真实验证的价值也包括发现产品不适用的地方。没有计时基线时，秒级生成清单
不能与完整的环境搭建、复现、编码和测试过程直接比较。

### 面试回答

> 我的案例没有只写成功故事。它证明了目标结构化和规范清单有用，
> 也证明当前检索不能替代 C++/SQL/Java 源码定位和完整 CI。

## 2026-09-07：Streamlit 基础界面
...
`st.session_state` 保存一次页面会话的交互对象，不等于长期持久化。
UI 代码负责输入和展示，领域校验、证据加载和评分放在 `ui_service.py`，
否则测试必须启动浏览器且业务规则会散落在控件回调中。

### 面试回答

> 我使用薄 UI + 应用服务分层。Streamlit 可以替换，核心 Agent、
> Pydantic 模型和离线测试不依赖页面框架。

## 2026-09-08：掌握度与准备度界面

### 完成内容

* 展示概念题、代码定位题和实践任务；
* 定位题确定性评分，概念题无模型时标记不确定；
* 实践任务始终进入人工复核；
* 展示掌握度、薄弱点、准备度分项和来源；
* 使用 SQLite 事务保存路线、结果和画像，并测试恢复一致性。

### 核心理解

页面进度条只是视图，真实进度来自可恢复的数据模型。
不可靠评分不能为了让 UI 好看而强行生成分数；`uncertain` 和
`needs_human_review` 本身也是有意义的业务状态。

### 面试回答

> 我区分页面 session_state、LangGraph checkpoint 和 SQLite 长期进度。
> 三者生命周期不同，不能用一个 dict 同时冒充会话恢复和持久化存储。

## 2026-09-09：V1.0 测试与演示材料

### 完成内容

* 补充 grounded Q&A、贡献准备度、openEuler 模式和 UI 服务离线测试；
* README 更新 V1.0 定位、Mermaid 架构和运行方式；
* 明确与 RepoMind 类通用仓库工具的产品边界；
* 补充真实案例、已知限制和五分钟演示脚本；
* 演示材料要求截图不包含 API Key、私有远程地址和个人目录。

### 核心理解

Agent 项目的测试重点不是“模型每次说同一句”，而是边界、结构、路由、
来源、拒答、分数公式和持久化等确定性契约。LLM 调用通过 Fake/Mock 隔离，
核心回归无需联网和付费。

### 面试回答

> RepoMentor 与通用仓库聊天工具的区别是：它围绕特定学习者和特定 Issue，
> 把限定证据、每日任务、测验、掌握度和贡献准备度连成闭环；
> 它不会为了回答更多问题而取消范围和拒答机制。
